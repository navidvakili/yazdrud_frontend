...
- **فایل:** `src/types.ts`
- **انواع:** `Course` (اکنون دارای `group_id: number | null` و `group_title: string | null`), `CourseRegistration`, `CourseSurvey`, `CourseSurveyStats`, `CourseCoupon`, `CourseStats`, `CourseGroup` (نوع جدید: `{ id, title, created_at, updated_at }`)
...
- `mapCourse(c)` — نگاشت `Course` به `TutCourse` (اکنون از `c.group_title` برای فیلد `category` استفاده می‌کند)
...
| زیرماژول | قابلیت‌ها |
|----------|-----------|
| **tuts-list** | گرید کارت‌ها با تصویر، جستجو، فیلتر دسته‌بندی، صفحه‌بندی، ایجاد/ویرایش/حذف دوره، فعال/غیرفعال کردن، دیالوگ آمار و جزئیات با جدول کامل ۱۳-ستونی (کد ملی، نام، شماره دانشجویی، موبایل، نوع کاربر، دوره، مبلغ، نوع پرداخت، شماره پیگیری، تاریخ ثبت‌نام، تاریخ تایید، وضعیت) + اسکرول مستقل |
...
- **افزودن گروه:** `handleAddCategory` — اعتبارسنجی (تکراری نباشد) ← `api.createCourseGroup(title)` ← به‌روزرسانی state
- **حذف گروه:** `handleDeleteCategory` — `confirm()` ← `api.deleteCourseGroup(id)` ← حذف از state
