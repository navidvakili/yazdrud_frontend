...
│   ├── functions.ts              # توابع کمکی (API, Upload, Download, BrowserFingerprint, ...)
│   ├── menuConfig.ts             # پیکربندی منو: انواع MenuCategory/SubmenuItem، نگاشت آیکون‌ها، توابع کمکی
│   └── networkStatus.ts          # ردیابی وضعیت اتصال به شبکه و سرور (Publish/Subscribe)
├── components/
...
│   ├── TutsModule.tsx            # ماژول مدیریت دوره‌های آموزشی (ارکستراتور نازک – ۷ زیرماژول + هوک‌های سفارشی)
...
│   │   ├── ToastNotification.tsx # کامپوننت اعلان Toast
│   │   ├── CourseFilterBar.tsx   # نوار فیلتر جستجو، دسته‌بندی و حالت نمایش
│   │   ├── CourseCard.tsx        # کارت دوره با نمایش گرید/لیست و کنترل‌های ادمین
...
│   ├── NetworkStatus.tsx         # نوار وضعیت اتصال به شبکه (ثابت پایین صفحه)
...
| **نوار وضعیت شبکه** | `components/NetworkStatus.tsx` | نمایش خودکار نوار قرمز در پایین صفحه هنگام قطع اینترنت یا عدم دسترسی به سرور |
...
flowchart TD
    ...
    App --> NetworkStatus[NetworkStatus<br/>self-contained]
...
| تابع | توضیح |
|------|-------|
| `API(URL, params, method)` | درخواست عمومی API — به همراه ردیابی خطاهای شبکه (گزارش به networkStatus) |
| `APISendFiles(URL, formData)` | آپلود فایل با multipart/form-data — به همراه ردیابی خطاهای شبکه |
| `downloadFile(URL, filename)` | دانلود فایل — به همراه ردیابی خطاهای شبکه |
...
## ردیابی وضعیت شبکه (Network Status)

### معماری

تشخیص خودکار قطعی اینترنت یا عدم دسترسی به سرور از طریق دو ماژول مجزا پیاده‌سازی شده است:

| لایه | فایل | مسئولیت |
|------|------|----------|
| **ردیاب مرکزی** | `lib/networkStatus.ts` | singleton با الگوی Publish/Subscribe — ردیابی `navigator.onLine` و رویدادهای `online`/`offline` + دریافت گزارش خطا/موفقیت از توابع API |
| **نمایشگر** | `components/NetworkStatus.tsx` | کامپوننت React که با subscribe به networkStatus، وضعیت را نمایش می‌دهد |

### نحوه عملکرد

1. **قطع اینترنت مرورگر:** با رویدادهای `window.addEventListener('online'/'offline')` و `navigator.onLine` تشخیص داده می‌شود
2. **عدم دسترسی به سرور:** توابع `API()`، `APISendFiles()` و `downloadFile()` در `lib/functions.ts` با try-catch خطاهای TypeScript ناشی از `fetch` را گرفته و به `networkStatus.reportApiFailure()` گزارش می‌دهند. درخواست موفق نیز `networkStatus.reportApiSuccess()` را صدا می‌زند
3. **خود-ترمیم‌شونده:** وضعیت apiFailing بعد از ۱۰ ثانیه به صورت خودکار ریست می‌شود تا اگر خطا موقتی بوده، نوار محو شود
4. **کامپوننت NetworkStatus** در `App.tsx` بعد از Footer قرار گرفته و در حالت عادی `null` برمی‌گرداند (چیزی نمایش نمی‌دهد). فقط هنگام بروز مشکل، نوار قرمز در پایین صفحه نمایش می‌یابد

### حالت‌های نمایش

| وضعیت | پیام | آیکون |
|-------|------|-------|
| `!online` | اتصال اینترنت قطع شده است | `WifiOff` + انیمیشن `Wifi` |
| `online && apiFailing` | ارتباط با سرور با مشکل مواجه شده است | `CloudOff` + انیمیشن `Wifi` |
| `online && !apiFailing` | **مخفی** (null) | — |
...
2. **فرانت‌اند (React):** کامپوننت `TutsModule.tsx` (جایگزین `CourseCoursework.tsx`) با ۷ زیرماژول، هوک‌های سفارشی، توابع کمکی و کامپوننت‌های UI استخراج‌شده

### بازمعماری (Refactoring) — استخراج زیرماژول‌ها، هوک‌ها و کامپوننت‌ها (۲ فاز)

فایل `TutsModule.tsx` در اصل یک فایل monolithic با حدود **۵۱۶۰ خط** بود که در دو فاز بازمعماری به چندین فایل کوچک تفکیک شد:

| فاز | توضیح | خطوط TutsModule | فایل‌های استخراج‌شده |
|-----|-------|-----------------|---------------------|
| **فاز ۱** | استخراج ۷ زیرماژول مجزا (فهرست، گزارش، فیش، آمار، نظرسنجی، آمار نظرسنجی، بن) + مودال‌های اشتراکی + کامپوننت‌های کوچک | ~2735 | ۱۱ فایل در `tuts/` |
| **رشد تدریجی** | افزوده‌شدن قابلیت‌های جدید (گواهی‌سازی، مدیریت مربی، آمار جزئی، پیش‌ثبت‌نام) | ~4355 | — |
| **فاز ۲** | استخراج هوک‌های سفارشی، توابع کمکی، انواع تایپ‌اسکریپت و کامپوننت‌های UI تکراری | ~3734 | `tuts-hooks.ts`، `tuts-utils.ts`، `tuts-types.ts`، `ToastNotification.tsx`، `CourseFilterBar.tsx`، `CourseCard.tsx` |
...
├── ToastNotification.tsx   # کامپوننت اعلان Toast
│                            #   نمایش خودکار پیغام‌های موفقیت/خطا با انیمیشن ظاهر/محو
├── CourseFilterBar.tsx     # نوار فیلتر جستجو و دسته‌بندی
│                            #   سرچ باکس، انتخاب دسته، سوئیچ گرید/لیست، دکمه‌های ادمین
├── CourseCard.tsx          # کارت دوره با دو حالت نمایش
│                            #   گرید: کارت با تصویر و اطلاعات خلاصه
│                            #   لیست: ردیف جدولی با کنترل‌های مدیریتی
...
### فاز ۱۴: مدیریت نشست‌های فعال (Active Sessions Management)

| ردیف | تغییر | فایل/مسیر | وضعیت |
|------|-------|-----------|-------|
| | **فرانت‌اند** | | |
| ۱ | افزودن نوع `ActiveSession`، `AdminSession`، `UserSessionsResponse`، `AdminSessionsResponse` | `src/types.ts` | ✅ |
| ۲ | افزودن ۴ متد API: `getActiveSessions()`, `revokeSession(tokenId)`, `getAllActiveSessions()`, `adminRevokeSession(tokenId)` + ارسال `browser_fingerprint` به لاگین | `src/api.ts` | ✅ |
| ۳ | افزودن بخش "نشست‌های فعال" به پروفایل کاربر — نمایش نشست فعلی (با نشان "نشست فعلی") و لیست سایر نشست‌ها شامل IP، مرورگر، سیستم‌عامل، تاریخ لاگین + دکمه خروج به تفکیک + دکمه "خروج از همه" + دکمه بروزرسانی | `src/components/ProfileModule.tsx` | ✅ |
| ۴ | ایجاد کامپوننت `AdminSessionsPanel` — نمایش تمام نشست‌های فعال کاربران به صورت جدول با قابلیت جستجو، مرتب‌سازی بر اساس نام کاربری، نمایش IP، مرورگر، سیستم‌عامل، تاریخ لاگین، نقش، نشان نشست فعلی + دکمه باطل کردن برای هر نشست (به جز نشست جاری مدیر) | `src/components/AdminSessionsPanel.tsx` | ✅ |
| ۵ | ثبت `AdminSessionsPanel` در ماژول رندرر `App.tsx` با شناسه `admin-sessions` | `src/App.tsx` | ✅ |
| ۶ | افزودن آیکون `Globe` به نگاشت FontAwesome → Lucide | `src/lib/menuConfig.ts` | ✅ |
| ۷ | ایجاد آیتم منوی "مدیریت نشست‌های فعال" برای نقش admin | `accesses` (دیتابیس) | ✅ |

---
**تاریخ آخرین به‌روزرسانی:** ۱۲ تیر ۱۴۰۵  
**نسخه:** 3.1.0
