...
class ApiService {
  ...
  async generateCertificate(registerId): Promise<Blob>
...
1. **دیالوگ پیش‌نمایش گواهی** — یک مودال تمام‌صفحه که گواهی را در `<iframe>` با استفاده از مسیر عمومی `{BACKEND_API_URL}/certificate/preview/{encryptedId}` نمایش می‌دهد و دکمه دانلود با لینک `{BACKEND_API_URL}/certificate/{encryptedId}` دارد.
...
| مسیر | توضیح |
|------|-------|
| `GET /certificate/{encryptedId}` | دانلود مستقیم PDF گواهی |
| `GET /certificate/preview/{encryptedId}` | نمایش گواهی در iframe (پیش‌نمایش) |
...
| GET | `/api/certificates/generate/{registerId}` | تولید PDF گواهی و دانلود |
...
---
**تاریخ آخرین به‌روزرسانی:** ۵ تیر ۱۴۰۵  
**نسخه:** 2.7.0
