...
│   ├── TutsModule.tsx            # مدیریت دوره‌های آموزشی (کامپوننت اصلی – ارسال زیرماژول‌ها)
...
فرم لاگین (`LoginForm.tsx`) با الهام از طراحی پرتال دموی مرکز کارآفرینی بازطراحی شده است:

- **پس‌زمینه:** گرادینت آبی-نیوی به نیلی (`#1e3a8a` → `#312e81`) مشابه `gold-gradient-bg` در دمو
- **کارد فرم:** شیشه‌ای (Glass-morphism) با `backdrop-blur-md` و حاشیه `border-white/20`
- **رنگ‌های اصلی:**
  - آبی نفتی (`#1e3a8a`) — رنگ اصلی برند
  - نارنجی طلایی (`#f97316`) — رنگ اکشن دکمه‌ها و تأکید (مشابه `brand-gold` در دمو)
  - سفید شفاف — برای تایپوگرافی و کاردها
- **المان‌های تزئینی:** اشکال SVG انتزاعی و دایره‌های محو (blur) در پس‌زمینه
- **لوگو:** تصویر `logo_nika.png` با فیلتر `brightness-0 invert` برای تطابق با پس‌زمینه تیره
- **حالت تاریک:** پس‌زمینه گرادینت ثابت (عدم استفاده از تم تیره مجزا)
- **فرمت فیلدها:** اینپوت‌های شیشه‌ای با `bg-white/10` و `backdrop-blur-sm`
...
│                            #   useToast, useTutsData, useCourseCategories, useCourseCRUD,
...
- **فایل محلی ماژول:** `src/components/tuts/tuts-types.ts` — انواع داخلی زیرماژول‌ها:
  ...
  - `TutRegistrant` — ثبت‌نام با `id`, `name`, `nationalCode`, `studentCode`, `mobile`, `typeText`, `courseId`, `courseTitle`, `date`, `verifiedAt`, `amount`, `paymentMethod`, `trackingCode`, `bankReceipt`, `status`, ...
...
|----------|----------|-----------|
| **tuts-list** | `TutsCourseList.tsx` | گرید کارت‌ها با تصویر، جستجو، فیلتر دسته‌بندی، صفحه‌بندی، ایجاد/ویرایش/حذف دوره، فعال/غیرفعال کردن، دیالوگ آمار و جزئیات با جدول کامل ۱۳-ستونی (کد ملی، نام، شماره دانشجویی، موبایل، نوع کاربر، دوره، مبلغ، نوع پرداخت، شماره پیگیری، تاریخ ثبت‌نام، تاریخ تایید، وضعیت) + اسکرول مستقل، پیش‌ثبت‌نام |
...
---
**تاریخ آخرین به‌روزرسانی:** ۴ تیر ۱۴۰۵  
**نسخه:** 2.4.0
